...
const loginValidation = (0, express_validator_1.body)('login')
    ...
    .isString().withMessage('Incorrect type of field \'login\' - must be string')
    .trim()
    .isLength({ min: 3, max: 10 }).withMessage('Field \'login\' must have length between 3 and 10 symbols')
    .matches('^[a-zA-Z0-9_-]*$')
    .withMessage('Field \'login\' contains inappropriate symbols');
const passwordValidation = (0, express_validator_1.body)('password')
    .exists().withMessage('Field \'password\' must be specified')
    .isString().withMessage('Incorrect type of field \'password\' - must be string')
    .trim()
    .isLength({ min: 6, max: 20 }).withMessage('Field \'password\' must  have length between 6 and 20 symbols');
const emailValidation = (0, express_validator_1.body)('email')
    .exists().withMessage('Field \'email\' must be specified')
    .isString().withMessage('Incorrect type of field \'email\' - must be string')
    .trim()
